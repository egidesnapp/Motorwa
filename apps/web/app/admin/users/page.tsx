...
export default function AdminUsersPage() {
  ...
  const handleBan = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      await fetch(`/api/v1/admin/users/${id}/ban`, {
        method: 'PUT',
  ...
  const handleUnban = async (id: string) => {
    try {
      const token = localStorage.getItem('accessToken');
      await fetch(`/api/v1/admin/users/${id}/unban`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleVerify = async (id: string) => {
    try {
      const token = localStorage.getItem('accessToken');
      await fetch(`/api/v1/admin/users/${id}/verify-id`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
      });
                ...
                <tr key={user.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {user.isIdVerified && <span className="px-2 py-0.5 text-xs rounded bg-success-pale text-success">Verified</span>}
                      {user.isBanned && <span className="px-2 py-0.5 text-xs rounded bg-error-pale text-error">Banned</span>}
                    </div>
                  </td>
