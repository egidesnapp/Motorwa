...
export default function AdminListingsPage() {
  ...
  const fetchListings = async () => {
    try {
      ...
      const res = await fetch(`/api/v1/admin/listings?status=${statusFilter}&page=1&limit=50`, {
  ...
  const handleApprove = async (id: string) => {
    try {
      const token = localStorage.getItem('accessToken');
      await fetch(`/api/v1/admin/listings/${id}/approve`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      await fetch(`/api/v1/admin/listings/${id}/reject`, {
        method: 'PUT',
        headers: {
          ...
          {listings.map((listing) => (
            <div key={listing.id} className="bg-white rounded-xl p-4 shadow-sm flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex sm:flex-col gap-2">
                <a href={`/cars/${listing.slug}`} target="_blank" className="btn-ghost flex items-center gap-1 text-sm">
                  <Eye size={16} /> View
                </a>
